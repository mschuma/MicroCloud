...
public class ChildHealthCheck implements Runnable {
	...
	static {
		String connectionRetriesAllowed = PropertiesHelper
				.getParentProperties()
				.getProperty(Constants.CONNECTION_RETRIES_ALLOWED,
		...
		try {
			...
			CONNECTION_RETRIES_ALLOWED = Integer
					.parseInt(connectionRetriesAllowed);
		} catch (NumberFormatException ex) {
			System.out.println("number format exception for the property :: "
					+ Constants.CONNECTION_RETRIES_ALLOWED);
			...
			while (!connected && attemptId <= CONNECTION_RETRIES_ALLOWED) {
				try {
					SSLSocketFactory f = (SSLSocketFactory) SSLSocketFactory
							.getDefault();
					childSocket = (SSLSocket) f.createSocket(this.child,
							this.childPort);
					connected = true;
					...
					// ex.printStackTrace();
					System.out.println("Ping attempt number " + attemptId
							+ " for child " + this.child + " is unsuccessful");
					attemptId++;
					try {
