...
public class ChildRegistrationThread implements Runnable {
	private static final String CLASS_NAME = ChildRegistrationThread.class
			.getCanonicalName();
	...
	public void run() {
		try {
			System.out.println("Communication from child :: "
			...
			System.out.println("Sent OK message to :: "
					+ socket.getInetAddress());
			String message = null;

			while ((message = reader.readLine()) != null) {
				System.out.println("Received registration message from :: "
						+ socket.getInetAddress());
				...
				if (msgType.equals(Constants.MSG_TYPE_REGISTER)) {
					System.out.println("Register child with VCL :: "
							+ socket.getInetAddress());
					registerResource();
					System.out
							.println("Finished registering child with VCL :: "
									+ socket.getInetAddress());
				} else if (msgType.equals(Constants.MSG_TYPE_UNREGISTER)) {
					System.out.println("Unregister child from VCL :: "
							+ socket.getInetAddress());
					unregisterResource();
					System.out
							.println("Finished unregistering child with VCL :: "
									+ socket.getInetAddress());
				}
				break;
			}
	...
	private void acknowledge() throws IOException {
		System.out.println("Sending acknowledgement to child :: "
				+ socket.getInetAddress());
		...
		System.out.println("Finished sending acknowledgement to child :: "
				+ socket.getInetAddress());
	}
